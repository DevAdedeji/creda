...
import { createError, getHeader, readMultipartFormData, setResponseStatus } from 'h3'
import { z } from 'zod'
...
export default defineEventHandler(async (event) => {
  ...
  const path = `businesses/${user.id}/${randomUUID()}.${format.extension}`
  ...
  try {
    ...
    console.error('Business image upload failed:', error instanceof Error ? error.message : error)
